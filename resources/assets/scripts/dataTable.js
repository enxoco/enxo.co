    ...
    $listItems.click(function(e) {
        ...
        .done(function (data) {
          if(!data) {
            ...
            console.log('no devices for this user')
          } else {
            $('.hasDevices').css('display','inherit')
            $('#devicesSelect').append(data[0].name)
            $('#deviceTable').html('')
            for(var x = 0; x <= data.length; x++) {

              if(data[x].isSupervised === true) {
                $('#deviceTable').append(`<tr>
                  <td>${data[x].name}</td>
                  <td>${data[x].serialNumber}</td>
                  <td>
...
$('#devicesSelect').each(function(){
    ...
    var table = $('#explicitSearchesTable').DataTable( {
      ...
      columns: [
          ...
          language: {
            emptyTable: "<center>There are no explicit searches so far this week",
    ...
    var table = $('#emailTopHitsTable').DataTable( {
      ...
      columns: [
          ...
          language: {
            emptyTable: "<center>Sorry there is no activity for " + userName + " so far this week",
            processing: loadingAnimation,
            info: "_PAGE_ of _PAGES_ pages"
          }
      ...
      columns: [
              ...
              },
              {
                ...
                render: function ( data, type, row, json ) {
                  ...
                  var query = ""
                  if(data.includes('google.com') && data.includes('q=')) {
                    var query = data.split('q=')
                    var query = query[1].replace('+', ' ')
                  }
              ...
              },
              {
                ...
                render: function(data, type, row) {
                  ...
                  if(row['pageTitle'] != row['URL']) {
                    return `<i class="ion ion-help-circled icon-info" data-html="true" data-title="
                    Date: ${row['Date']}

                    This page was: ${row['Action']}ed

                    The full url is: ${row['URL']}

                    Words matching the following categories were found on this page:
                      ${row['catsBlocked']}
                      ">
                  </i>
                    `
                  ...
                  return `<i class="ion-help-circled icon-info"
                            data-html="true"
                            data-title="Date: ${row['Date']}

                            The title of the page is: ${row['pageTitle']}
